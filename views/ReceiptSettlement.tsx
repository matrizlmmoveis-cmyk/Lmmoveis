...
const ReceiptSettlement: React.FC<ReceiptSettlementProps> = ({ sales, setSales, employees, stores }) => {
    ...
    const pendingSettlements = deliveryPayments.filter(dp =>
        dp.payment.status === 'PENDENTE_ENTREGA' || dp.payment.status === 'AGUARDANDO_ACERTO'
    );
    ...
    const renderCard = (dp: any, isConferido: boolean) => (
        <div key={`${dp.sale.id}-${dp.payment.amount}`} className={`p-5 rounded-2xl border mb-4 font-bold ${isConferido ? 'bg-slate-50 border-slate-200' : 'bg-white border-blue-100 shadow-sm'}`}>
            <div className="flex justify-between items-start mb-3">
                ...
                <div className="text-right">
                    ...
                    <span className={`text-[10px] uppercase px-2 py-0.5 rounded-full inline-block mt-1 ${dp.payment.status === 'AGUARDANDO_ACERTO' ? 'bg-amber-100 text-amber-700' :
                            dp.payment.status === 'PENDENTE_ENTREGA' ? 'bg-slate-100 text-slate-500' :
                                'bg-emerald-100 text-emerald-700'
                        }`}>
                        {dp.payment.status === 'PENDENTE_ENTREGA' ? 'Na Rua (Motorista)' : dp.payment.status === 'AGUARDANDO_ACERTO' ? 'Aguardando Acerto' : dp.payment.status}
                    </span>
                </div>
            </div>
            ...
            {!isConferido && dp.payment.status === 'AGUARDANDO_ACERTO' && (
                <div className="mt-4 pt-4 border-t border-slate-100 flex justify-end">
                    <button
                        onClick={() => handleConfirmar(dp.sale.id, dp.payment.amount)}
                        className="bg-emerald-600 hover:bg-emerald-700 text-white px-5 py-2.5 rounded-xl text-xs uppercase shadow-md shadow-emerald-500/20 transition-all flex items-center gap-2"
                    >
                        <CheckCircle2 className="w-4 h-4" /> Confirmar Recebimento em Caixa
                    </button>
                </div>
            )}
        </div>
